<?php
$row = $list;
$actilyse_data = $this->asm_model->find_actilyse_data($row->VEEVA_Employee_ID, $this->input->get('id'));
?>
...
<div class="panel panel-default">
    <div class="panel-body">-->
        ...
        <div class="col-lg-12">
          ...
          <table class="table  table-bordered panel ">
                <tr>
                    <th>Multi Super Seciality</th>
                    <th> No. of Emergency Beds</th>
                    <th>No Of Beds</th>
                    <th>In house   CT Scan facility</th>
                    <th>In house MRI facility  </th>
                    <th>CT Scan timing (24x7 or specific timing)</th>
                    <th> Owned / liaised Ambulance service</th>
                </tr>
                <tr>
                    <td><?php
                    if (!empty($actilyse_data)) {
                        echo $actilyse_data->Multi_Super_speciality;
                    }
                    ?></td>
                    <td>
                        <?php
                        if (!empty($actilyse_data)) {
                            echo $actilyse_data->No_of_Emergency_Beds;
                        }
                        ?></td>    
                    ...
                    <td><?php
                        if (!empty($actilyse_data)) {
                            echo $actilyse_data->CT_Scan24;
                        }
                        ?></td> 
                    <td><?php
                        if (!empty($actilyse_data)) {
                            echo $actilyse_data->Ambulance_service;
                        }
                        ?></td> 
                </tr>
...
for ($i = 1; $i <= 12; $i++) {
    ...
    echo '<th>' . $month . '</th>';
    $MONTH1 = $this->asm_model->month1($this->input->get('id'), $i, $row->VEEVA_Employee_ID);
...
for ($i = 1; $i <= 12; $i++) {
    ...
    $activities = $this->asm_model->activity_reportmonth($i, $this->input->get('id'), $row->VEEVA_Employee_ID);
    if (!empty($activities)) {
