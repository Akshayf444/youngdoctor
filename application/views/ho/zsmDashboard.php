        ...
        if (!empty($productlist2)) {
            ...
            foreach ($productlist2 as $product) {
                ...
                if (isset($_GET['Product_Id']) && $_GET['Product_Id'] > 0) {
                    ?>
                    <li class="<?php echo isset($_GET['Product_Id']) && $_GET['Product_Id'] == $product->id ? 'active' : ''; ?>"><a style="padding: 12px;" href="<?php echo site_url('Report/ZSMdashboard?Product_Id=' . $product->id); ?>" ><?php echo $product->Brand_Name ?></a></li>

                <?php } else { ?>
                    <li class="<?php echo isset($count) && $count == 1 ? 'active' : ''; ?>"><a style="padding: 12px;" href="<?php echo site_url('Report/ZSMdashboard?Product_Id=' . $product->id); ?>" ><?php echo $product->Brand_Name ?></a></li>
                                ...
                                <div id="<?php echo $product->id ?>11" class="tab-pane fade <?php echo isset($count) && $count == 1 ? 'in active' : ''; ?>">
                                    ...
                                    if (!empty($Status)) {
                                        ...
                                        foreach ($Status as $value) {
                                            $currentMonthRx['Actual_Rx'] = !is_null($value->current_month) ? $value->current_month : 0;
                                            $LastMonthRx['Actual_Rx'] = !is_null($value->last_month_Rx) ? $value->last_month_Rx : 0;
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        if (!empty($dashboardDetails)) {
                                            foreach ($dashboardDetails[$product->id] as $value) {
                                                ...
                                                . '<td>' . number_format($value[9], 2, '.', '') . '</td>'
                                                . '<td>' . number_format($value[10], 2, '.', '') . '</td></tr>';
                                                $kpi1 += $value[9];
                                                $kpi2 += $value[10];
                                            }
                                        }

                                        echo '<tr><th>Total</th><td>' . number_format($kpi1, 2, '.', '') . '</td><td>' . number_format($kpi2, 2, '.', '') . '</td></tr>';
                                        ?>
                                    </table>
                                </div>
                                <?php
